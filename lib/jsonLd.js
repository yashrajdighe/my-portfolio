...
/** Canonical Medium profile — overrides any other medium.com URL from the API in sameAs. */
const MEDIUM_PROFILE_URL = "https://medium.com/@yashraj45dighe";
...
export const personSchema = (basics) => ({
  ...
  sameAs: (() => {
    const urls = (basics?.profiles ?? [])
      .map((p) => p.url)
      .filter(Boolean);
    const withoutMedium = urls.filter((u) => !/medium\.com/i.test(u));
    return [...withoutMedium, MEDIUM_PROFILE_URL];
  })(),
});
